...
void mainStateMachine(void)
{
        ...
        else
        {
            if (biItsAcqBusy)
            {
                if (bSmExecCnt >= CONVERSION_TIME)
                {
                    ...
                    miCurrTempDegCx10 = TempSensor_CalculateTempCx10(
                            (int) mawAdcMeasurements[0], mawAdcMeasurements[1]);
                    eState = S_UPDATE_DISP1;
                    //break;
                }
                else
    ...
    case S_CHECK_BUTTON:
        ...
        else
        {
            if(iDebounceCnt >= DEBOUNCE_TIME+1)
            {
                ...
                //eState = S_CHECK_ADC;
                //break;
                if(iDebounceCnt == LONGPRESS_TIME) //long press
                {
                    switch(iBtnState)
                    {
                        case 1:
                        if(miSetValueDegCx10 < SETTEMPCX10_MAX-10)
                        {
                            miSetValueDegCx10 += 10;
                        }
                        ...
                        break;
                        case 2:
                        if(miSetValueDegCx10 > SETTEMPCX10_MIN+10)
                        {
                            miSetValueDegCx10 -= 10;
                        }
                        miSettingTimeTempDeltaCx10 = miSetValueDegCx10 - miCurrTempDegCx10; // update the setting time temperature difference
                ...
                else if(iDebounceCnt > LONGPRESS_TIME)
                {
                    iDebounceCnt = DEBOUNCE_TIME+1;
                }
                else
                {
    ...
    case S_CONTROL:
        if (mbiActive)
        {
            ...
            if (miCurrTempDegCx10
                    > (miSetValueDegCx10 + mainAbs(miContolDeadBandCx10)
                            + miRefrigirateDeadBandOffsetCx10))
            {
                if (iControlDebounceCnt >= CONTROL_DEBOUNCE_TIME)
                {
                    // Cool down
                    if (1/*(miSetValueDegCx10 < (TempSensor_CalculateChipTemp(mawAdcMeasurements[1])/10 + miRefrigirateHeatDisableTempBandCx10)) || !mbiTargetReached*/)
                    {
            ...
            else
            {
                ...
                else
                {
                    if (iControlDebounceCnt <= 0)
                    {
                        ...
                        if (miCurrTempDegCx10
                                > (miSetValueDegCx10 - miMinDeadBandDegCx10)
                                && miCurrTempDegCx10
                                        < (miSetValueDegCx10
                                                + miMinDeadBandDegCx10))
                        {
